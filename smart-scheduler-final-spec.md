...
- `routine` 슬롯에는 루틴만 배치 가능
- `project` 슬롯에는 프로젝트만 배치 가능
- `any` 슬롯에는 루틴 + 프로젝트 모두 배치 가능
- 루틴이 항상 프로젝트보다 먼저 배치됨
- Focus가 Buffer보다 먼저 배치됨

### 13.6 설정 페이지 기능

**시간 자동 정렬**: 시간대를 추가하거나 수정하면 자동으로 시간순 정렬됩니다.
